...
class VariablePLC:
    ...
    def __init__(self, name, opc_adr, plc: Client, scale: str, ID,is_archive:str):
        self.name = name
        self.ID = ID
        self.opc_adr = opc_adr
        self.plc = plc
        self.scale = SCALE_LIST[scale]
        self._is_archive = is_archive == "True"
...
if __name__ == '__main__':
    ...
    for dt in data:
        var_list.add(VariablePLC(
            dt["name"], f'{ADR}.{dt["opc_adr"]}', plc_1,dt["scale"] , dt["ID"], dt["isArchive"]))
